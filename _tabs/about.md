---
# Default layout is 'page'
icon: fas fa-info-circle
order: 4
---

## Hey 👋🏽, I'm [Lillian!](https://lillianphyo.github.io/)

📍 **Location:** Yangon, Myanmar  
📧 **Email:** khinpyaephyosan@gmail.com  
💼 **Current Role:** Lead Developer @ VPTechnologies  
🌱 **Passionate about:** System Integration, AI/ML, Scalable Systems

### About Me

Dynamic **Software Engineer** with **5 years of experience** in front-end design, OSS-BSS systems, system development, and AI/ML. I specialize in designing and implementing **scalable systems**, optimizing system response times, and developing autonomous AI-driven solutions. 

📌 **Fav Quote:** _"Make things happen. Adventurous and realistic. Happy-Go-Lucky. Stubborn optimist!♊"_

---

## 💻 Skills

- **Front-end:** HTML5, CSS3, TypeScript, jQuery, Bootstrap, Django, ShadCN
- **Back-end:** Python, Flask, Node.js, Express, TypeORM, SQLAlchemy
- **Web Services:** REST, GraphQL, Web API
- **Testing & Documentation:** Postman, Swagger
- **Databases:** MySQL, MariaDB, MongoDB, MSSQL, CockroachDB, MindsDB
- **DataOps:** Apache NiFi, Mage.ai, Python
- **MLOps:** Neptune.ai, Prefect.io, MLflow, Kubeflow
- **ETL Frameworks:** Apache Airflow, Pandas, PySpark, Apache Hadoop
- **Machine Learning:** PyTorch, TensorFlow, Scikit-learn, Keras
- **DevOps & Cloud:** Docker, Kubernetes, AWS, GCP, CI/CD, Serverless Architecture

---

## 📜 Work Experience

### VPTechnologies, Myanmar | **Lead Developer** *(08/2024 - Present)*
- Developed **scalable OSS-BSS solutions** for telecommunications.
- Optimized **system performance** for high-volume transactional data.
- Implemented **AI-driven analytics** for business intelligence and operational efficiency.

### EcoCodeX, Myanmar | **Full Stack Developer** *(08/2024 - 08/2024)*
- Designed and deployed **scalable web applications** with enhanced security.
- Built **RESTful APIs** and integrated third-party services.

### Digrodart, Myanmar | **Full Stack Developer** *(01/2021 - 07/2022)*
- Developed **agritech data analytics** platforms.
- Implemented **ML models** for crop yield forecasting.

### MyanmarAPN, Myanmar | **System Development Engineer** *(03/2020 - 03/2023)*
- Engineered **scalable network solutions** for ISP services.
- Integrated **data-driven insights** for network performance optimization.

---

## 📚 Education

- 🎓 **Master’s Degree in Computer Science** *(2024)* - University of Information Technology
- 🎓 **Bachelor’s Degree in Computer Science** *(2017)* - University of Information Technology

---

## 📖 Publications

- **[DLOps: Developing Deep Learning Aided Burmese Rice Price Forecasting](https://doi.org/10.1109/ICAIT65209.2024.10754945)** *(11/2024)*
- **[Cloud Cost Prediction with LSTM on Parametric Data](https://doi.org/10.1007/978-981-96-1531-5_50)** *(02/2025)*

---

## 📜 Certifications

- Certified Kubernetes Administrator
- Certified Calico Administrator (Tigera.io)
- Harness Certified Continuous Delivery & GitOps Developer
- MLOps Zoom Camp 2024 (DataTalksClub)
- Machine Learning Zoom Camp 2024 (DataTalksClub)

---

## 🌍 Languages

- 🇲🇲 Burmese - Native
- 🇬🇧 English - C1
- 🇯🇵 Japanese - N5

---

## 🏆 Featured Projects

### **MLOps & AI Systems**
- Sustainable **self-adaptive scratch MLOps pipelines**.
- **AWS ML system for rice price forecasting** using LSTM & BiLSTM.
- **Data pipeline for commodity rice price** using Apache NiFi & Airflow.

### **Software & Web Development**
- SaaS for **OSS-BSS services** using Node.js, TypeScript, MariaDB, and Redis.
- **CI/CD for Kubernetes infrastructure** using Flux, Flagger & GitHub Actions.

---

⭐️ From [Lillian Phyo](https://github.com/lillianphyo)

